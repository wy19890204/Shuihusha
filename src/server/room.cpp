...
void Room::onPhaseEnd(ServerPlayer *player){
    if(!player)
        return;
    if(player->hasFlag("drank")){
        ...
        setPlayerFlag(player, "-drank");
    }
    foreach(ServerPlayer *tmp, getAllPlayers()){
        if(tmp->hasFlag("ecst")){
            LogMessage log;
            ...
            log.to << tmp;
            sendLog(log);
            setPlayerFlag(tmp, "-ecst");
        }
        if(tmp->hasFlag("Guibing"))
            setPlayerFlag(tmp, "-Guibing");
    }
    ...
    if(!Config.BanPackages.contains("events")){
        ServerPlayer *source = findPlayerWhohasEventCard("jiefachang");
        if(source && player == source){
            ...
            if(face)
                askForUseCard(player, "Jiefachang", "@jiefachang");
        }
    }

    // zhuan shi
    if(Config.EnableReincarnation){
        int count = Sanguosha->getPlayerCount(getMode());
        if(count < 4)
            return;
        int max = count > 5 ? 4 : 3;
        ...
        while(next->isDead()){
            if(next->getHandcardNum() >= max){
                ...
                broadcastInvoke("playAudio", "mode/reincarnation");
                revivePlayer(next);
                ...
                if(!Config.value("ReincaPersist", false).toBool()){
                    ...
                    if(!names.isEmpty()){
                        ...
                        transfigure(next, newname, false, true, oldname);
                    }
                }
                if(next->getMaxHp() == 0)
                    setPlayerProperty(next, "maxhp", 1);
                setPlayerProperty(next, "hp", 1);

                getThread()->delay(1500);
                attachSkillToPlayer(next, "sacrifice");
                setPlayerMark(next, "@skull", 1);
                setPlayerProperty(next, "isDead", true);
            }
            next = next->getNext();
        }
    }
