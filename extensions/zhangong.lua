...
--[[
zgfunc[sgs.CardMoving].htdl=function(self, room, event, player, data,isowner,name)
	...
	if table.contains(from_places,sgs.Player_PlaceHand) and move.to_place==sgs.Player_PlaceHand
			and move.to:objectName()==room:getOwner():objectName() and move.from:getKingdom()=='kou' then
		for i=1,#ids,1 do
...
-- jdfy :: 绝对防御 :: 在一局游戏中，使用甲马累计出闪20次
--
zgfunc[sgs.FinishJudge].jdfy=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
...
--
zgfunc[sgs.PhaseChange].jg=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
...
--
zgfunc[sgs.GameStarted].thy=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	for _,cd in sgs.qlist(player:getHandcards()) do
...
zgfunc[sgs.CardResponsed].xcdz=function(self, room, event, player, data,isowner,name)
	if data:toCard():isKindOf("Jink") and isowner then
		addGameData(name,1)
		if getGameData(name)==20 then addZhanGong(room,name) end
	end
end
...
zgfunc[sgs.HpRecover].xhjs=function(self, room, event, player, data,isowner,name)
	...
	if player:getHp()<=0 and recover.recover+player:getHp()>=1 and recover.who
			and recover.who==room:getOwner() and isSameGroup(player,recover.who) then
		addGameData(name,1)
...
--
zgfunc[sgs.CardMoving].ymds=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	local move=data:toCardMove()
	if move.from_places == sgs.Player_Hand and move.from == room:getOwner()
		and move.to_place == sgs.Player_Equip and move.to == room:getOwner() then
		if sgs.Sanguosha:getCard(move.card_id):isKindOf("Horse") then
			addGameData(name,1)
...
--
zgfunc[sgs.CardMoving].gzwb=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.CardMoving].jyh=function(self, room, event, player, data,isowner,name)
	if room:getOwner():getGeneralName()~="bulianshi" then return false end
...
--
zgfunc[sgs.PhaseChange].dym=function(self, room, event, player, data,isowner,name)
...
-- dym :: 大姨妈 :: 使用甄姬连续5回合洛神的第一次结果都是红色，不包括改判
--
zgfunc[sgs.FinishJudge].dym=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~='zhenji' then return false end
	if not isowner then return false end
	local judge=data:toJudge()
...
--
zgfunc[sgs.FinishJudge].glnc=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.FinishJudge].qmjj=function(self, room, event, player, data,isowner,name)
...
zgfunc[sgs.CardMoving].tmnw=function(self, room, event, player, data,isowner,name)
...
zgfunc[sgs.FinishJudge].tmnw=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~='simayi' then return false end
	local judge=data:toJudge()
...
--
zgfunc[sgs.CardMoving].tmzf=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~='simayi' then return false end
	local move = data:toMoveOneTime()
...
zgfunc[sgs.FinishJudge].tmzf=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~='simayi' then return false end
	local judge=data:toJudge()
...
--
zgfunc[sgs.FinishJudge].xzxm=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.FinishJudge].ajnf=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.CardMoving].gmzc=function(self, room, event, player, data,isowner,name)
	local move=data:toMoveOneTime()
...
--
zgfunc[sgs.FinishJudge].lgzw=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~='zhangjiao' then return false end
	local judge=data:toJudge()
...
--
zgfunc[sgs.PhaseChange].zsbsh=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.FinishJudge].tyzm=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.FinishJudge].lbss=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	local judge=data:toJudge()
...
--
zgfunc[sgs.PhaseChange].ydqb=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
...
zgfunc[sgs.HpChanged].dsdnx=function(self, room, event, player, data,isowner,name)
	if room:getMode()~="dusong" or not player:isLord() then return false end
...
zgfunc[sgs.CardFinished].yfnzmk=function(self, room, event, player, data,isowner,name)
	...
	local use=data:toCardUse()
	local card=use.card
	if card:isKindOf("YijieCard") then setTurnData(name,1) end
end
...
--
zgfunc[sgs.DamageProceed].ljxs=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	local damage = data:toDamage()
...
zgfunc[sgs.Damaged].mbgj=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	local damage = data:toDamage()
	if damage and damage.card and damage.card:isKindOf("Lightning") and player == room:getCurrent() then
		setTurnData(name,1)
	end
end
...
-- mbgj :: 命不该绝 :: 被闪电劈中但是没有死
--
zgfunc[sgs.PhaseChange].mbgj=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
...
--
zgfunc[sgs.FinishJudge].jlsj=function(self, room, event, player, data,isowner,name)
		local judge=data:toJudge()
...
--
zgfunc[sgs.FinishJudge].alg=function(self, room, event, player, data,isowner,name)
		local judge=data:toJudge()
	if judge.reason=="indulgence" and judge.who:objectName()==room:getOwner():objectName()
...
--
zgfunc[sgs.FinishJudge].bjlz=function(self, room, event, player, data,isowner,name)
		local judge=data:toJudge()
...
--
zgfunc[sgs.FinishJudge].jcll=function(self, room, event, player, data,isowner,name)
		local judge=data:toJudge()
	if judge.reason=="supply_shortage" and judge.who:objectName()==room:getOwner():objectName()
			and judge:isBad() then
		addGameData(name,1)
...
--
zgfunc[sgs.FinishJudge].tq=function(self, room, event, player, data,isowner,name)
	local judge=data:toJudge()
	if judge.reason=="lightning" and room:getTag("retrial"):toBool()==false
			and judge.who:objectName()==room:getOwner():objectName() then
		setTurnData(name,1)
...
-- yqt :: 一骑讨 :: 与人决斗胜利累计30次
--
zgfunc[sgs.Predamage].yqt=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	local damage = data:toDamage()
...
--
zgfunc[sgs.Predamage].bszj=function(self, room, event, player, data,isowner,name)
	local damage = data:toDamage()
...
--
zgfunc[sgs.CardMoving].dtj=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	local move=data:toMoveOneTime()
...
--
zgfunc[sgs.FinishJudge].bmjs=function(self, room, event, player, data,isowner,name)
	local judge=data:toJudge()
	if judge.reason=="eight_diagram" and judge.who==room:getOwner() then
		if judge:isBad() then
			setGameData(name,0)
		else
			addGameData(name,1)
...
zgfunc[sgs.Death].yzzf=function(self, room, event, player, data,isowner,name)
	local damage = data:toDamageStar()
	if damage and damage.card and damage.card:isKindOf("SavageAssault") and room:getOwner()==room:getCurrent() then
...
zgfunc[sgs.Death].jwxf=function(self, room, event, player, data,isowner,name)
	local damage = data:toDamageStar()
	if damage and damage.card and damage.card:isKindOf("ArcheryAttack") and room:getOwner()==room:getCurrent() then
		local key=name..damage.card:getEffectiveId()
		addTurnData(key,1)
		if getTurnData(key)==3 then
			addZhanGong(room,name)
...
zgfunc[sgs.Death].zszm=function(self, room, event, player, data,isowner,name)
	...
	if (player:getRole()=="rebel" or player:getRole()=="renegade") and damage and damage.from
			and damage.from==room:getOwner() and damage.from:isLord() then
		addGameData(name,1)
	end
end
...
--
zgfunc[sgs.Predamaged].dshx=function(self, room, event, player, data,isowner,name)
	if not isowner then return false end
	local damage = data:toDamage()
...
zgfunc[sgs.SlashEffect].fj=function(self, room, event, player, data,isowner,name)
	if room:getOwner():getGeneralName()~="lvfang" then return false end
	if not isowner then return false end
	local effect= data:toSlashEffect()
...
zgfunc[sgs.Death].fj=function(self, room, event, player, data,isowner,name)
	...
	local damage=data:toDamageStar()
	if damage and damage.from and damage.from:objectName()==room:getOwner():objectName()
		and damage.from:getGeneralName()=="lvfang" and damage.card:hasFlag(name) then
		addGameData(name,1)
		if getGameData(name)==2 then
			addZhanGong(room,name)
		end
...
--
zgfunc[sgs.CardMoving].lsdjx=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.FinishJudge].lsf=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~="zhenji" then return false end
	if not isowner then return false end
	local judge=data:toJudge()
	if judge.reason=="luoshen" and judge.who:objectName()==room:getOwner():objectName() then
...
--
zgfunc[sgs.FinishJudge].qjtj=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.PhaseChange].kcjc=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.CardMoving].sjdf=function(self, room, event, player, data,isowner,name)
	local move=data:toMoveOneTime()
...
--
zgfunc[sgs.CardMoving].yhdf=function(self, room, event, player, data,isowner,name)
	if room:getOwner():getGeneralName()~="sunshangxiang" then return false end
...
zgfunc[sgs.PhaseChange].fcdc=function(self, room, event, player, data,isowner,name)
	if not isowner or player:getGeneralName()~="xiahouyuan" then return false end
...
--
zgfunc[sgs.PhaseChange].bfh=function(self, room, event, player, data,isowner,name)
	if room:getMode()~="06_3v3" then return false end
	if not isowner then return false end
...
zgfunc[sgs.CardMoving].bfh=function(self, room, event, player, data,isowner,name)
	if room:getMode()~="06_3v3" then return false end
...
--
zgfunc[sgs.GameStarted].mlgr=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.GameStarted].ymgr=function(self, room, event, player, data,isowner,name)
	if not isowner or room:getMode()~="02_1v1" then return false end
	local list = room:getOwner():getTag("1v1Arrange"):toStringList()
	local n=0
	for _, generalname in ipairs(list) do
...
--
zgfunc[sgs.GameStarted].jgyx=function(self, room, event, player, data,isowner,name)
	if not isowner or room:getMode()~="02_1v1" then return false end
	local list = room:getOwner():getTag("1v1Arrange"):toStringList()
	local n=0
	for _, generalname in ipairs(list) do
...
--
zgfunc[sgs.GameStarted].hgjs=function(self, room, event, player, data,isowner,name)
	if not isowner or room:getMode()~="02_1v1" then return false end
...
--
zgfunc[sgs.GameStarted].yywm=function(self, room, event, player, data,isowner,name)
	if not isowner or room:getMode()~="02_1v1" then return false end
	local list = table.concat(room:getOwner():getTag("1v1Arrange"):toStringList(),",")
	local n=0
	list=list..","..player:getGeneralName()
	for _, generalname in ipairs({"lvfang","likui","muhong"}) do
		if string.match(list,generalname) then n=n+1 end
	end
	if n==3 then
		setGameData(name,1)
...
--
zgfunc[sgs.GameStarted].zysq=function(self, room, event, player, data,isowner,name)
	if not isowner or room:getMode()~="02_1v1" then return false end
	local list = table.concat(room:getOwner():getTag("1v1Arrange"):toStringList(),",")
	local n=0
	list=list..","..player:getGeneralName()
	for _, generalname in ipairs({"guansheng","linchong","huarong"}) do
		if string.match(list,generalname) then n=n+1 end
	end
	if n==3 then
		setGameData(name,1)
...
--
zgfunc[sgs.CardMoving].wwd2=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.CardMoving].djlj=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.CardMoving].qqqz=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.CardMoving].fwjj=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~='erzhang' then return false end
	local move=data:toMoveOneTime()
	if room:getCurrent():objectName()~=room:getOwner():objectName() then return end
	local reason=move.reason.m_reason
	if reason==sgs.CardMoveReason_S_REASON_USE and move.reason.m_skillName=="zhijian" and move.to:getKingdom()=='min'
			and isowner and room:getCurrent():objectName()==room:getOwner():objectName() then
		addGameData(name,1)
		if getGameData(name)==5 then
			addZhanGong(room,name)
...
zgfunc[sgs.PhaseChange].mrgs=function(self, room, event, player, data,isowner,name)
	if  room:getOwner():getGeneralName()~='sunce' then return false end
	if not isowner then return false end
...
--
zgfunc[sgs.CardMoving].bzcq=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.CardMoving].dcyq=function(self, room, event, player, data,isowner,name)
...
--
zgfunc[sgs.FinishJudge].xltj=function(self, room, event, player, data,isowner,name)
...
zgfunc[sgs.HpRecover].wgzm=function(self, room, event, player, data,isowner,name)
	...
	local recov = data:toRecover()
	if not recov.card and player:getHp()==0 and recov.who:objectName()==room:getOwner():objectName() and player:getKingdom()=='min' then
		if getGameData(name,0)==0 then setGameData(name,"") end
...
zgfunc[sgs.GameOverJudge].callback.styj=function(room,player,data,name,result)
	if  room:getOwner():getGeneralName()~="panjinlian" then return false end
	if result=='win' and room:getLord():getGeneralName()=='ximenqing' and room:getOwner():getRole()=='renegade' then
		addZhanGong(room,name)
	end
end
...
zgfunc[sgs.GameOverJudge].callback.wjjh=function(room,player,data,name,result)
	if  room:getOwner():getGeneralName()~="yanxijiao" then return false end
	if result=='win' and room:getLord():getGeneralName()=='songjiang' and room:getOwner():getRole()=='renegade' then
		addZhanGong(room,name)
	end
end
...
--
zgfunc[sgs.GameStarted].gsy=function(self, room, event, player, data,isowner,name)
...
--[[
zgfunc[sgs.TurnStart].dusong=function(self, room, event, player, data,isowner,name)
	if room:getMode()=="dusong" and player:isLord() and player:getMark("secondMode")==1 and player:getGeneral2() and player:getMark("changedusong2")==0 then
		if player:hasSkill("wushuang") then room:detachSkillFromPlayer(player, "wushuang") end
		room:setPlayerMark(player, "changedusong2", 1)
	...
	if room:getMode()=="dusong" and player:isLord() and player:getMark("secondMode")==1 and not player:faceUp() then
		player:turnOver()
	end
end
]]
...
function usedusongCard(room,owner)
	if room:getMode()~='dusong' or not owner:askForSkillInvoke("usedusongCrad") then return false end
	local generalnames=sgs.Sanguosha:getRandomGenerals(998)

	local banlist={"ubuntenkei", "catty"}
